...
export function RecipeBrowser({ catalog, chooseOnly = false }: { catalog: Recipe[]; chooseOnly?: boolean }) {
  ...
  return <main className="recipe-browser"><header>{chooseOnly && <a className="back-link" href="#/choose">返回点菜</a>}<p className="eyebrow">{chooseOnly ? '点菜模式' : '家庭菜谱'}</p><h1>{chooseOnly ? '选菜' : '今天吃什么'}</h1><p className="subtitle">{chooseOnly ? '只选想吃的菜，不做其他修改。' : '搜索菜名、食材或标签，安排今天的一餐。'}</p><label>搜索食谱<input aria-label="搜索食谱" role="searchbox" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜菜名、食材或标签" /></label></header>

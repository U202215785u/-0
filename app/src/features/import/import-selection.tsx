...
export function ImportSelection({ catalog }: { catalog: Recipe[] }) {
  ...
  const importCode = async () => {
    try {
      const ids = decodeSelection(text).recipeIds.filter((id) => catalog.some((recipe) => recipe.id === id))
      if (!ids.length) throw new Error('没有识别到本地食谱')
      await appDb.imports.put({ id: `import-${Date.now()}`, recipeIds: ids })

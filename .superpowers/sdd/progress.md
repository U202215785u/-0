...
- Data integrity: commit aecae0f — deleted char-siu.json (a second transcription of the same RHS page as easy-char-siu.json and a 叉烧 title collision with the MWL recipe); added mango-chicken-summer-rolls.json transcribed from the live Bon Appétit page's structured data; quality gate now flags duplicate titles/source URLs and buildCatalog rejects id/filename mismatches; INDEX.md synced to 57 rows.
- Behavior review: commit 8442956 — planner retry now repeats the failed remove intent instead of re-adding the dish; 清除已购 also clears bought manual items; choose-mode detail returns to the browse list with a new 返回点菜 entry; cooking countdown shows 分/秒; 44px touch targets for steppers/remove/card actions; PWA manifest start_url/scope; e2e locators use exact text.
- Current verification: Vitest 83/83 with zero stderr, oxlint 0/0, tsc clean, production build clean, Playwright 18/18.
- Round 3 (fresh-eyes sweep): fixed-quantity recipes hide the misleading servings stepper (4d0871c); apple-touch-icon uses the non-maskable icon; share-code decoding edge cases locked by tests (48e0bbf). Review covered every source module, domain logic, catalog data invariants (id/filename, duplicate title/source, unit mix, duration/servings outliers), PWA manifest/offline, a11y touch targets, and e2e locator ambiguity; no further defects found.

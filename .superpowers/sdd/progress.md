...
- Behavior review: commit b3241ea (pending) — planner retry now repeats the failed remove intent instead of re-adding the dish; 清除已购 also clears bought manual items; choose-mode detail returns to the browse list with a new 返回点菜 entry; cooking countdown shows 分/秒; 44px touch targets for steppers/remove/card actions; PWA manifest start_url/scope; e2e locators use exact text.
- Current verification: Vitest 80/80 with zero stderr, oxlint 0/0, tsc clean, production build clean, Playwright 18/18.
